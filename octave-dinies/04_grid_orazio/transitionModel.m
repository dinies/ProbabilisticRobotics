  ...
  #against each other cell and itself
	...
	translation_cols = col - col_from_;
	    ...
	    if (target_row < 1 || target_row > map_rows || target_col < 1 || target_col > map_cols) #if we're going over the border
		    invalid_motion = true;
	    elseif (map_(target_row, target_col) == 1 || map_(row, col) == 1) #obstacle in the goal cell
		    invalid_motion = true;
	    endif
	    if (invalid_motion)
          ...
          transition_probability_matrix(row, col) = 1; #we stay with 100% probability (no motion has full confidence)
		      continue;
	      else
      ...
      #our motion is feasible - compute resulting transition
	    switch (control_input_)
		    case MOVE_UP 
			    if (translation_rows     ==  1 && translation_cols ==  0) transition_probability_matrix(row, col) = 0.8;
          ...
          elseif (translation_rows ==  1 && translation_cols == -1) transition_probability_matrix(row, col) = 0.1;
		    ...
		    case MOVE_LEFT
			    if (translation_rows     ==  0 && translation_cols == -1) transition_probability_matrix(row, col) = 0.8;
			    elseif (translation_rows ==  1 && translation_cols == -1) transition_probability_matrix(row, col) = 0.1;
			    elseif (translation_rows == -1 && translation_cols == -1) transition_probability_matrix(row, col) = 0.1;
			    endif;
		    case MOVE_RIGHT

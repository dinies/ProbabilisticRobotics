...
function [mu, sigma, id_to_state_map, state_to_id_map, last_landmark_id] = correction(mu, sigma, observations, id_to_state_map, state_to_id_map, last_landmark_id)
  ...
  mu_robot = mu(1:3);
  mu_t = mu_robot(1:2); % translational part of the robot pose
  mu_theta = mu_robot(3); % rotation of the robot
  ...
  for i=1:num_landmarks_measured
    ...
    if(state_pos_of_landmark != -1) 
      ...
      [measure_prediction, C_m] = measurement_function(state_dim, mu_robot, landmark_mu, landmark_state_vector_index);

...
function [mu, sigma, id_to_state_map, state_to_id_map] = correction(mu, sigma, observations, id_to_state_map, state_to_id_map)
  ...
  for i=1:num_landmarks_measured
    ...
    if(state_pos_of_landmark != -1) 
      ...
      delta_t= landmark_mu - mu_t;  #distance between the robot and the landmark (in world coordinates)
      measure_prediction = Rt*delta_t;   #relative distance to the landmark, seen from the robot
  ...
  if ((num_old_landmarks_measured > 0))
    ...
    sigma_z = sigma_w^2*eye(2*num_old_landmarks_measured);

    #Kalman gain
    K = sigma*C_t'*inverse( sigma_z + C_t*sigma*C_t' );
  ...
  for i=1:num_landmarks_measured
    ...
    # where we see the landmark
    z_t =[ measurement.x_pose ;  measurement.y_pose] ;
